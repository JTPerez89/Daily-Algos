...
function merge(left, right) {
    ...
    for(i=0; i<left.length + right.length; i++) {
        if(left[leftCount] < right[rightCount] && leftCount < left.length || rightCount >= right.length) {
            arr.push(left[leftCount]);
            leftCount++;
        } else if(rightCount < right.length) {
            arr.push(right[rightCount]);
            rightCount++;
        }
    }
    return arr;
 ...
 * split the array in 2 until everything is in its own array
 */
function mergeSort(nums) {
    if(nums.length == 1){
        return nums;
    }
    const numsMid = Math.floor(nums.length/2);
    const numsLeft = nums.slice(0, numsMid);
    const numsRight = nums.slice(numsMid);
