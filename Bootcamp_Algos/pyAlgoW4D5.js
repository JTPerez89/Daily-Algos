 ...
 */
function lowestCommonMultiple(a, b, min = Math.min(a, b), max = Math.max(a, b)) {
    if (max % min == 0) {
        return max;
    }
    return lowestCommonMultiple(a, b, min, max += Math.max(a, b))
}
...
You are given aSTRING containing chars "0", "1", and "?"
